 ...
 * 设置非阻塞
 * 成功返回监听套接字
 *创建套接字，绑定，监听   socket  bind listen 
 */
int tcp_server(const char *host, unsigned short port)
{
	...
	if (host != NULL) {
		if (inet_aton(host, &servaddr.sin_addr) == 0) {  //一个字符串IP地址转换为一个32位的网络序列IP地址
            //如果是主机名，根据主机名，得到主机的ip地址    gethostbyname
        	struct hostent *hp;
		...
		servaddr.sin_addr.s_addr = htonl(INADDR_ANY);   //绑定的是主机的所有ip地址
